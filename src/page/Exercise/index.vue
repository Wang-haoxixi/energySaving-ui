<template>
  <div class="allScrean">
    ...
    <end-page :data="endData" v-if="endFlag" @review="handleReview"></end-page>
  </div>
</template>
<script>
import { answerExe, editExe } from "@/api/qms/exam_exercise_answer";
import answerArea from "@/views/exam/examList/Page/Components/answerArea";
import answerTime from "./Components/answerTime";
import { answerHandle, paperHand } from "@/api/qms/exam_exercise_answer";
import {
  getTestResultById,
  checkState,
...
export default {
  ...
  methods: {
    ...
    async handleCheckState() {
      const { data } = await checkState(this.id, 2);
      console.log(data);
    ...
    async load() {
      if (this.state === "continue") {
        const { data } = await editExe(this.id);
