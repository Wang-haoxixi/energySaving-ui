...
function readNodes(nodes: TreeNode[], filterFunction: Function) {
...
const getTextByTreeNode = (nodes: TreeNode[], valueArr: Array<string | number>, strArr: string[] = []): string[] => {
  ...
  if (idx >= 0) {
    ...
    if (valueArr.length === 1) {
      ...
      if (nodes[idx].children !== undefined) {
        ...
        return getTextByTreeNode(nodes[idx].children!, tail(valueArr), strArr);
      } else {
        return strArr;
      }
    }
