...
import EchartsTheme from "./EchartsTheme-Woneng";
...
export default {
  ...
  methods: {
    ...
    init(options) {
      ...
      // echarts.init(dom, 'woneng')
      // console.log(this.theme);
      // 载或复制以下的主题保存至 *.json 文件；
      // 读取该 JSON 文件，并使用 obj = JSON.parse(data) 将其转换成对象；
      // 调用 echarts.registerTheme('woneng', obj) 注册主题；
      // 使用 echarts.init(dom, 'woneng') 创建图表，第二个参数即为刚才注册的主题名字。
      this.registerTheme(this.theme, EchartsTheme);
