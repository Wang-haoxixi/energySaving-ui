...
const defaultConfig = {
    ...
    strokeColorM: '#7ecef4',     // 头像外围包裹的颜色主
    strokeColorP: '#7ecef4',     // 头像外围包裹的颜色人
    strokeColorS: '#7ecef4',     // 头像外围包裹的颜色SO
    strokeOpacity: .5,               //头像外圈透明度
    strokeWidth: 3,             // 头像外围包裹的宽度
    strength: 0.2,                  //碰撞力
    iterations: 5,               //碰撞迭代次数
    handleAvatarClick: null,        //头像点击事件
};
...
export default class RelationChart {
    ...
    constructor(selector, data, configs = {}) {
        ...
        const mapW = parseInt(d3.select(selector).style('width'));
        const mapH = parseInt(d3.select(selector).style('height'));
        ...
        // 创建力学模拟器
        this.initSimulation();
        this.highlightObject(this.config.nodes[0]);
    }
    ...
    initSimulation() {
        ...
        this.links = this.edges.append("path").attr("class", "links")
            ...
            .attr('stroke', (d) => {
                const str = d.color ? "#" + d.color : this.config.linkColor;
                return str;
            });
        ...
        this.texts = this.rect_g.append("text")
            .attr("x", 40)
            .attr("y", 5 + (this.config.relFontSize - 12) / 2)
            .attr("text-anchor", "middle")  // <text>文本中轴对齐方式居中  start | middle | end
            .style("font-size", this.config.relFontSize).text(d => {
                return d.relation;
            });
        ...
        this.circles = this.relMap_g.selectAll("circle.circleclass")
            ...
            .attr("stroke", (d) => {
                const str = d.color ? "#" + d.color : (d.index === 0 ? this.config.strokeColorM : (d.type === 0 ? this.config.strokeColorP : this.config.strokeColorS));
                return str;
            })
            ...
            .on('mouseout', function () {
                const oMain = that.config.nodes[0];
                // d3.select(this).attr('stroke-width', that.config.strokeWidth);
                // d3.select(this).attr('stroke', '#c5dbf0');
                if (that.config.isHighLight) {
                    that.highlightObject(null);
                    that.highlightObject(oMain);
                }
            })
            .on('click', function (d) {
                const cb = that.config.handleAvatarClick;
                cb && cb(d);
                // console.log('头像节点click');
                // 展示方式2 ：浮窗展示
                // eslint-disable-next-line no-global-assign
                event = d3.event || window.event;
                // const pageX = event.pageX ? event.pageX : (event.clientX + (document.body.scrollLeft || document.documentElement.scrollLeft));
                // const pageY = event.pageY ? event.pageY : (event.clientY + (document.body.scrollTop || document.documentElement.scrollTop));
