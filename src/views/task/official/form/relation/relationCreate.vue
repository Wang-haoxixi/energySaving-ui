  ...
  <div class="relation">
    ...
    <div class="relation-list" v-show="!isSearch" v-if="!isCreate">
      ...
      >
        ...
        <div class="item" v-for="(item, index) in list[card.value]" :key="`${card.value}-${index}`">
          <div class="title">
            <i :class="card.icon"></i>
            <span>{{item.name}}</span>
          </div>
    ...
    <div class="search-list" v-show="isSearch">
      ...
      <div class="search-items" v-loading="loadIng">
        <!-- 材料 -->
        <div class="content-list material" v-if="activeName === 'material'">
          <el-checkbox-group v-model="formData.material" class="table-content">
        ...
        <div class="content-list summary" v-if="activeName === 'summary'">
          <el-checkbox-group v-model="formData.summary" class="table-content">
        ...
        <div class="content-list activity" v-if="activeName === 'activity'">
          <el-checkbox-group v-model="formData.activity" class="table-content">
        ...
        <div class="content-list questionnaire" v-if="activeName === 'questionnaire'">
          <el-checkbox-group v-model="formData.questionnaire" class="table-content">
    ...
    <!-- 关联弹框 -->
    <RelationDialog ref="relation" @relation-submit="relationSubmit"></RelationDialog>
  </div>
</template>

<script>
import { tabList, tabData, initParams, initList } from "./options";
...
export default {
  ...
  methods: {
    ...
    handleSubmit() {
      this.isSearch = true;
      this.handleClick({ name: "material" });
    },
    handleClick(row) {
      this.loadIng = true;
      this.searchFn[row.name](
        Object.assign({}, this.params[row.name], {
        ...
        this.loadIng = false;
        this.tabData[row.name] = data.records;
      });
    },
    handleRelation() {
    ...
    dealData(list) {
      ...
      for (const item of list) {
        this.list[annexesType[item.type]].push(item);
        this.formData[annexesType[item.type]].push(item.id);
      }
    },
  },
};
