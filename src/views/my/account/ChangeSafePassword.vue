<template>
  <div class="change-form">
    ...
    >
      ...
      <iep-form-item label-name="新支付密码" prop="newPassword" class="iep-form-half-block">
        <el-input
...
import { resetSafePassword } from "@/api/admin/user";
...
export default {
  ...
  data() {
    ...
    const validateConfirmPassword = (rule, value, callback) => {
      if (value === "") {
        callback(new Error("请确认您输入的新支付密码"));
  ...
  methods: {
    ...
    async submitForm() {
      try {
        ...
        const { data } = await resetSafePassword(form);
        if (data) {
          this.$message.success(
            "支付密码修改成功！请重新登录，3秒后即刻跳转到登录页",
          );
