<template>
  <iep-basic-container class="router-box">
    <iep-page-header :title="$route.name" :backOption="backOption">
      <template slot="left">
        <iep-tip
          content="信用值是节能保平台依据用户在平台中的各项行为数据，运用云计算和机器学习等技术，对各维度数据进行综合处理和评估，在用户身份特质、人脉关系、履约行为、行为偏好和历史信用五个维度客观呈现用户信用状况的综合分值。致力于解决用户与用户，用户与组织之间的信任问题，构建互惠互信的节能保世界。"
        ></iep-tip>
      </template>
    </iep-page-header>
    <div class="wrap">
        ...
        <div class="dashboard">
          ...
          <div class="detail-wrap">
            ...
            <div>
              信用值等级说明
              <iep-tip
                content="节能保新用户初始信用分为380分，之后随着自身行为而变化。用户信用分为五个层级：<br>
                750-1000  信用极好；<br>
                700-750  信用优秀；<br>
                650-700  信用良好；<br>
                600-650  信用中等；<br>
