  ...
  <div class="dynamic" ref="dynamic">
      ...
      <div v-if="list.length > 0">
        <div class="dynamic-item" v-for="(item, index) in list" :key="index">
          <div class="item-title">
            ...
            <div class="avatar">
              <iep-img :src="item.avatar" class="avatar-img" :resizeOpt="{l:24}"></iep-img>
            </div>
          ...
          >
            <div class="reply-item" v-for="(reply, rIndex) in replyList" :key="rIndex">
              <div class="reply-title">
                <div class="avatar">
                  <iep-img :src="reply.createAvatar" class="avatar-img" :resizeOpt="{l:24}"></iep-img>
                </div>
    ...
    <div class="dynamic-comment">
      <el-input
        ...
        maxlength="250"
        show-word-limit
        :class="{'isSpace': isSpace}"
      ></el-input>
      <div class="button-list">
        <!-- 上传附件的按钮 -->
        <i class="el-icon-woneng-fujian" @click="handleUpload"></i>
        <!-- 隐藏的上传组件 -->
        <iep-upload
          ref="upload"
          v-show="false"
          v-model="formData.commentAnnexes"
          :limit="99"
          multiple
          action="/api/admin/file/upload"
        ></iep-upload>
        <!-- 附件的列表 -->
        <div class="upload-list" v-if="formData.commentAnnexes.length === 1">
          <div class="upload-item nowrap">{{formData.commentAnnexes[0].name}}</div>
          <i class="el-icon-close" @click.stop="handleDeleteUpload(0)"></i>
        </div>
        <el-dropdown
          class="upload-list"
          v-if="formData.commentAnnexes.length > 1"
          placement="bottom-start"
        >
          <div class="el-dropdown-link">
            <div class="dropdown-title nowrap">{{formData.commentAnnexes[0].name}}</div>
            <i class="el-icon-arrow-down el-icon--right"></i>
          </div>
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item v-for="(item, index) in formData.commentAnnexes" :key="index" class>
              <div class="dropdown-item nowrap">{{item.name}}</div>
              <i class="el-icon-close" @click.stop="handleDeleteUpload(index)"></i>
            </el-dropdown-item>
          </el-dropdown-menu>
        </el-dropdown>
        <!-- 评论的提交按钮 -->
...
import { typeList, initCommentForm } from "./options";
...
export default {
  ...
  data() {
    return {
      ...
      formData: initCommentForm(),
      isSpace: false, // 评论非空判断值
    };
  },
  methods: {
    loadData(id) {
    ...
    handleSubmit() {
      ...
      row.commentAnnexes = row.commentAnnexes.map(m => {
        return {
          attachFile: m.url,
          original: m.name,
        };
      });
      if (row.commentAnnexes.length > 0) {
        row.type = 5;
      }
      createComment(row).then(() => {
        this.pageOption.current = 1;
        this.loadPage(true);
        this.formData = initCommentForm();
      });
    ...
    handleDeleteUpload(index) {
      this.formData.commentAnnexes.splice(index, 1);
    },
    handleDownload(row) {
      downloadFile({
        url: row.attachFile,
