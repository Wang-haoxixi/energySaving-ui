<template>
  <div>
    <div v-show="pageState === 'list'">
      <iep-basic-container>
        <iep-page-header title="其他数据"></iep-page-header>
        <div style="width:100%;">
        ...
        >
          <template #degree="{scope}">{{dictsMap.degree[scope.row.degree] || "暂无"}}</template>
          <template #status="{scope}">{{dictsMap.status[scope.row.status] || "暂无"}}</template>
          <template #mold="{scope}">{{dictsMap.mold[scope.row.mold] || "暂无"}}</template>
