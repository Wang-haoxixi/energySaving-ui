<template>
  <iep-dialog title="拜师申请" :dialogShow="dialogShow" @close="dialogShow=false" width="500px">
...
<script>
import { createMentor } from "@/api/admin/mentor";
const initForm = () => {
  return {
    remarks: "",
    masterId: "0",
...
export default {
  ...
  methods: {
    ...
    async submitForm() {
      const { data } = await createMentor(this.form);
      if (data) {
