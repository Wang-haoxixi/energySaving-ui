<template>
  <div class="category-wrapper">
    <div v-if="categoryOption.typeList.length" class="classify-item">
      <span class="name">类型：</span>
      <div class="classify-con">
        <span
          :class="{active: String(item.id) === searchForm[typeName||'type']}"
    ...
    <div v-if="currentItem.length" class="classify-item">
      ...
      <div class="classify-con">
        <span
          :class="{active: String(item.id) == searchForm.category}"
...
export default {
  ...
  computed: {
    categoryOptionMap() {
      const { typeList } = this.categoryOption;
      return mapKeys(typeList, "id");
    },
    type() {
      return this.searchForm.type;
    },
    currentItem() {
      return this.categoryOptionMap[this.type].child || [];
    },
  },
  methods: {
    handleSearch(i, p) {
