  ...
  <div class="left">
    <div class="head">
      ...
      <div class="icon">
        ...
        <div class="code" @click="handleCollect">
          <i class="el-icon-woneng-heart"></i>收藏
        </div>
        <div class="code">
    ...
    <el-card class="card">
      ...
      <el-button type="primary" class="button" @click="handleJoin" v-if="!formData.authority">加入任务</el-button>
      <el-button
        type="primary"
        class="button"
        @click="handleFinish(formData)"
        v-else-if="formData.state === '1'"
      >标记完成</el-button>
      <el-button type="primary" class="button" v-else>已完成</el-button>
    </el-card>
    <div class="content">
      <!-- 描述 -->
      <div class="desc form-item">
        ...
        <div class="form-span">
          <pre>{{formData.description || '暂无'}}</pre>
        </div>
      </div>
      <!-- 协作人 -->
    ...
    <div class="comment-tpl" v-if="formData.authority">
      <CommentTpl :id="formData.id"></CommentTpl>
    </div>
    <!-- 奖励分配 -->
...
import { getTaskUser, applyJoin } from "@/api/qms/cooperation_task";
...
export default {
  ...
  methods: {
    loadData(id) {
      getDetailById({ taskId: id }).then(({ code, data }) => {
        if (code === 1) {
          this.$router.go(-1);
          return false;
        }
        ...
        row.cooperationTaskUserVOList = row.cooperationTaskUserVOList.map(
          (m) => {
            m.id = m.userId;
