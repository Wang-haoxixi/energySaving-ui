<template>
  <div class="newest-tpl">
    <div class="head">最近浏览</div>
    <div class="list" v-if="list.length > 0">
      <div class="data" v-for="(item, index) in list" :key="index" @click="handleDetail(item)">
        <div class="title">
          <div class="name" :title="item.recruitName">{{transformString(item.recruitName)}}</div>
        ...
        <div class="footer" :title="item.orgName">{{item.orgName}}</div>
      </div>
    </div>
...
import { getRecruitHistory } from "@/api/dms/recruit_history";
