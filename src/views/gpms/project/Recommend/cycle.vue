<template>
  <div>
    <h3 style="text-align: center;font-size:14px;">项目平均耗时：{{timePerDataStr}}天</h3>
    <iep-charts class="echarts" :options="options" autoresize />
  </div>
</template>
<script>
import { averageTime } from "@/api/gpms/index";
export default {
  props: {
    projectTagList: {
      type: Array,
  ...
  methods: {
    loadData() {
      averageTime({ projectTagList: this.projectTagList }).then(res => {
        ...
        this.timePerDataStr = res.timePerDataStr;
      });
    },
  },
};
